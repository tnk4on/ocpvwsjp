...
const getCurrentTime = () => {
    ...
    try {
        ...
        const remoteCommandA = `sudo ssh root@192.168.123.100 -o StrictHostKeyChecking=no -q 'reboot'`;
        ...
        for (let i = 0; i < ipAddresses.length; i++) {
            ...
            // Bastionノードの再起動
            console.log(`[${getCurrentTime()}] -----Reboot Bastion node...`);
            execSync(`sshpass -p "${nowPASS}" ssh -o StrictHostKeyChecking=no -q lab-user@${nowIP} "${remoteCommandA}"`);
